...
public class ChangePassControl extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        int id = a.getId();
        String tmppass = a.getPass();
        String oldpass = request.getParameter("oldpass");
        String newpass = request.getParameter("newpass");
        String cfpass = request.getParameter("confirmpass");
        DAO dao = new DAO();
        if(tmppass.equals(oldpass)){
            if(newpass.equals(cfpass)){
                dao.ChangePassWord(newpass, id);
                request.getRequestDispatcher("logout").forward(request, response);
            }
            else{
                if(newpass!= null){
                    request.setAttribute("mess", "Mật khẩu lặp lại không chính xác");
                }
